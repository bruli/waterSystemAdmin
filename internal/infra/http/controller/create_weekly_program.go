...
func CreateWeeklyProgram(set *pongo3.TemplateSet, zonesSvc *zones.FindZones, createSvc *programs.CreateWeekly, log zerolog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPost {
			processCreateWeeklyForm(r, context, createSvc)
		}
...
func processCreateWeeklyForm(r *http.Request, context map[string]interface{}, svc *programs.CreateWeekly) {
	if err := r.ParseForm(); err != nil {
		context["error_msg"] = "invalid form"
		return
	}
	day, err := programs.ParseWeekDay(r.FormValue("day"))
	if err != nil {
	...
	if err = svc.Create(r.Context(), &weekly); err != nil {
		context["error_msg"] = err.Error()
		return
	}
